...
col_3=[
    ...
    [sg.Text('保存先フォルダー')],
    [sg.FileBrowse('Browse', file_types=(('空キャピラリーのchiファイル', '*.chi'),)), sg.InputText(key='capipath')],
    [sg.Text('ファイル名'), sg.InputText(key='savename'), sg.Button('グラフを保存', key='btn_F')]
]
...
def plot():
    ...
    dfw['I']=dfw['I']/IC2_w
    dfw_q=dfw.query('1<q<3')
    dfc=fi.read_chi(value['capipath'])
    dfc['I']=dfc['I']/IC2_c
    dfc_q=dfc.query('1<q<3')



    ax.scatter(dfw['q'], dfw['I'], c='r', s=1, label='water', alpha=0.5)
    ax.scatter(dfc['q'], dfc['I'], c='b', s=1, label='capi', alpha=0.5)
    ax.scatter(dfw['q'], dfw['I']-dfc['I'], c='g', s=1, alpha=0.5)
    ...
    F=sum(dfw_q['I']-dfc_q['I'])/len(dfw_q['q'])
